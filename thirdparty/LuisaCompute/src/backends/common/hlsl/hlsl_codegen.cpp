...
void StringStateVisitor::visit(const CastExpr *expr) {
    ...
    switch (expr->op()) {
        case CastOp::STATIC:
            ...
            break;
        case CastOp::BITWISE: {
            auto from_type = expr->expression()->type();
            auto to_type = expr->type();
            while (to_type->is_vector()) {
                to_type = to_type->element();
            }
            while (from_type->is_vector()) {
                from_type = from_type->element();
            }
            if (from_type->tag() == Type::Tag::FLOAT16 || to_type->tag() == Type::Tag::FLOAT16) {
                if (from_type->tag() == Type::Tag::UINT32 && to_type->tag() == Type::Tag::FLOAT16) {
                    str << "(float16_t)f16tof32"sv;
                }
                else if (from_type->tag() == Type::Tag::FLOAT16 && to_type->tag() == Type::Tag::UINT16) {
                    str << "(uint16_t)f32tof16"sv;
                }
                else{
                    LUISA_ERROR_WITH_LOCATION(
                        "Bitwise cast from '{}' to '{}' not implemented.",
