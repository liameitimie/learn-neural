...
/// EASTL_`EUE_DEFAULT_NAME
///
/// Defines a default container name in the absence of a user-provided name.
///
#ifndef EASTL_PRIORITY_QUEUE_DEFAULT_NAME
#define EASTL_PRIORITY_QUEUE_DEFAULT_NAME \
	EASTL_DEFAULT_NAME_PREFIX " priority_queue" // Unless the user overrides something, this is "EASTL priority_queue".
#endif
...
#define EASTL_PRIORITY_QUEUE_DEFAULT_ALLOCATOR allocator_type(EASTL_PRIORITY_QUEUE_DEFAULT_NAME)
	...
	template <typename T,
	          typename Container = eastl::vector<T>,
	...
	public:
		...
		// typedef typename Container::allocator_type           allocator_type;  // We can't currently declare this
		// because the container may be a type that doesn't have an allocator.
	...
	public: // We declare public so that global comparison operators can be implemented without adding an inline level
		    // and without tripping up GCC 2.x friend declaration failures. GCC (through at least v4.0) is poor at
		    // inlining and performance wins over correctness.
		container_type c;
		// The C++ standard specifies that you declare a protected member variable of type Container called 'c'.
		compare_type comp; // The C++ standard specifies that you declare a protected member variable of type Compare
		                   // called 'comp'.

	public:
		priority_queue();

		// Allocator is templated here because we aren't allowed to infer the allocator_type from the Container, as some
		// containers (e.g. array) don't have allocators. For containers that don't have allocator types, you could use
		// void or char as the Allocator template type.
		void reserve(size_t capacity);
		template <class Allocator>
		explicit priority_queue(
		    const Allocator& allocator,
		    typename eastl::enable_if<eastl::uses_allocator<container_type, Allocator>::value>::type* = NULL)
		    : c(allocator), comp()
		{
		}

		template <class Allocator>
		priority_queue(const this_type& x,
		               const Allocator& allocator,
		               typename eastl::enable_if<eastl::uses_allocator<container_type, Allocator>::value>::type* = NULL)
		...
		priority_queue(std::initializer_list<value_type> ilist,
		               const compare_type& compare = compare_type()); // C++11 doesn't specify that std::priority_queue
		                                                              // has initializer list support.
		...
		void pop(value_type&
		             value); // Extension to the C++11 Standard that allows popping a move-only type (e.g. unique_ptr).

		void change(
		    size_type n); /// Moves the item at the given array index to a new location based on its current priority.
	...
	inline priority_queue<T, Container, Compare>::priority_queue()
	    : c()
	    , // To consider: use c(EASTL_PRIORITY_QUEUE_DEFAULT_ALLOCATOR) here, though that would add the requirement that
	      // the user supplied container support this.
	    comp()
	{
	}
	...
	template <typename T, typename Container, typename Compare>
	inline priority_queue<T, Container, Compare>::priority_queue(std::initializer_list<value_type> ilist,
	...
	template <typename T, typename Container, typename Compare>
	template <typename InputIterator>
	inline priority_queue<T, Container, Compare>::priority_queue(InputIterator first,
	                                                             InputIterator last,
	                                                             const compare_type& compare)
	...
	template <typename T, typename Container, typename Compare>
	inline typename priority_queue<T, Container, Compare>::size_type priority_queue<T, Container, Compare>::size() const
	{
		return c.size();
	}
	...
	template <typename T, typename Container, typename Compare>
	inline typename priority_queue<T, Container, Compare>::const_reference priority_queue<T, Container, Compare>::top()
	    const
	{
		return c.front();
	}
	...
	{
		push(value_type(
		    eastl::forward<Args>(args)...)); // The C++11 Standard 23.6.4/1 states that c.emplace is used, but also
		                                     // declares that c doesn't need to have an emplace function.
	}
	...
	template <typename T, typename Container, typename Compare>
	inline void priority_queue<T, Container, Compare>::change(
	    size_type n) // This function is not in the STL std::priority_queue.
	{
	...
	template <typename T, typename Container, typename Compare>
	inline void priority_queue<T, Container, Compare>::remove(
	    size_type n) // This function is not in the STL std::priority_queue.
	{
	...
	template <typename T, typename Container, typename Compare>
	inline void priority_queue<T, Container, Compare>::reserve(size_t capacity)
	{
		c.reserve(capacity);
	}
	...
	template <typename T, typename Container, typename Compare>
	inline void priority_queue<T, Container, Compare>::swap(this_type& x)
	    EA_NOEXCEPT_IF((eastl::is_nothrow_swappable<this_type::container_type>::value &&
	                    eastl::is_nothrow_swappable<this_type::compare_type>::value))
	{
		using eastl::swap;
		swap(c, x.c);
	...
	template <typename T, typename Container, typename Compare>
	inline bool priority_queue<T, Container, Compare>::validate() const
	{
	...
	template <typename T, typename Container, typename Compare>
	inline bool operator!=(const priority_queue<T, Container, Compare>& a,
	...
	inline void swap(priority_queue<T, Container, Compare>& a, priority_queue<T, Container, Compare>& b) EA_NOEXCEPT_IF(
	    (eastl::is_nothrow_swappable<typename priority_queue<T, Container, Compare>::container_type>::value &&
	     eastl::is_nothrow_swappable<typename priority_queue<T, Container, Compare>::compare_type>::
	         value)) // EDG has a bug and won't let us use Container in this noexcept statement
	{
		a.swap(b);
	}
